...
def add_loan():
    ...
    try:
        ...
        try:
            # Set owner to the authenticated user_id
            upload_options = {
                "content-type": "application/pdf",
                "owner": user_id  # Explicitly set owner to match user_id
            }
            supabase.storage.from_("loan-files").upload(
                aadhar_filename, aadhar_file_content, upload_options
            )
            supabase.storage.from_("loan-files").upload(
                pan_filename, pan_file_content, upload_options
            )
        except Exception as e:
