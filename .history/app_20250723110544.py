...
SUPABASE_KEY = os.getenv("SUPABASE_KEY")  # Should be Anon Key for client-side auth
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not auth_response.user:
                flash("Failed to create user in Supabase Auth.", "error")
                return render_template('sign-up.html')

            user_data['id'] = auth_response.user.id  # Use Supabase Auth user ID
            ...
            if response.data:
                ...
                session['jwt_token'] = auth_response.session.access_token
                logger.info(f"User signed up: {username}, role: {sign_up_as}, user_id: {user_data['id']}")
...
def signin():
    if request.method == 'POST':
        ...
        try:
            ...
            if user_data['sign_up_as'] == sign_in_as:
                session.permanent = True
                session['user_id'] = user_data['id']
                session['username'] = user_data['username']
                session['sign_in_as'] = sign_in_as
                session['jwt_token'] = auth_response.session.access_token
                logger.info(f"User signed in: {user_data['username']}, role: {sign_in_as}, user_id: {user_data['id']}")
                flash("Signed in successfully!", "info")
                return redirect(url_for('dashboard'))
            else:
                flash("Incorrect role.", "error")
    ...
    if role == 'merchant':
        try:
            ...
            if 'jwt_token' in session:
                supabase.auth.set_auth(session['jwt_token'])
...
def add_loan():
    ...
    if 'jwt_token' in session:
        ...
        logger.error("No JWT token in session")
        flash("Authentication token missing. Please sign in again.", "error")
        return jsonify({'error': 'Authentication token missing'}), 401

    # Debug: Log current Supabase user
    try:
        current_user = supabase.auth.get_user()
        logger.info(f"Current Supabase user: {current_user.user.id if current_user.user else 'None'}")
        if current_user.user.id != session['user_id']:
            logger.error(f"User ID mismatch: session={session['user_id']}, auth={current_user.user.id}")
            flash("User authentication mismatch. Please sign in again.", "error")
            return jsonify({'error': 'User authentication mismatch'}), 401
    except Exception as e:
        logger.error(f"Error verifying Supabase user: {str(e)}")
        flash("Authentication error. Please sign in again.", "error")
    ...
    if not all([full_name, age, monthly_income, occupation, loan_amount, loan_purpose, aadhar_number, pan_number, aadhar_file, pan_file]):
        logger.error("Missing required fields for loan application")
        flash("All fields, including Aadhar and PAN files, are required.", "error")
        return jsonify({'error': 'All fields, including Aadhar and PAN files, are required'}), 400
    ...
    if aadhar_file and (not aadhar_file.filename.endswith('.pdf') or aadhar_file.content_length > 5 * 1024 * 1024):
        logger.error("Invalid Aadhar file: Must be a PDF and less than 5MB")
        flash("Aadhar file must be a PDF and less than 5MB.", "error")
        return jsonify({'error': 'Invalid Aadhar file'}), 400
    if pan_file and (not pan_file.filename.endswith('.pdf') or pan_file.content_length > 5 * 1024 * 1024):
        logger.error("Invalid PAN file: Must be a PDF and less than 5MB")
        flash("PAN file must be a PDF and less than 5MB.", "error")
        return jsonify({'error': 'Invalid PAN file'}), 400

    try:
        # Generate unique file names
        loan_id = str(uuid.uuid4())
        aadhar_filename = f"loans/{loan_id}/aadhar_{aadhar_number}.pdf"
        pan_filename = f"loans/{loan_id}/pan_{pan_number}.pdf"

        # Upload files to Supabase storage bucket
        bucket_name = "loan-documents"
        aadhar_file.seek(0)  # Reset file pointer
        pan_file.seek(0)     # Reset file pointer

        # Upload Aadhar file
        aadhar_upload = supabase.storage.from_(bucket_name).upload(
            path=aadhar_filename,
            file=aadhar_file.read(),
            file_options={
        ...
        logger.info(f"Aadhar file uploaded: {aadhar_filename}")

        # Upload PAN file
        pan_upload = supabase.storage.from_(bucket_name).upload(
            path=pan_filename,
            file=pan_file.read(),
            file_options={
                "content-type": "application/pdf",
        ...
        logger.info(f"PAN file uploaded: {pan_filename}")

        # Get public URLs for the uploaded files
        aadhar_url = supabase.storage.from_(bucket_name).get_public_url(aadhar_filename)
        pan_url = supabase.storage.from_(bucket_name).get_public_url(pan_filename)

        # Insert loan data with file URLs
        loan_data = {
            'loan_id': loan_id,
            'user_id': user_id,
            'full_name': full_name,
            ...
            'aadhar_file_url': aadhar_url,
            'pan_file_url': pan_url,
            'status': 'Pending',
            'created_at': datetime.utcnow().isoformat()
        }
...
def get_loans():
    if 'user_id' not in session:
        logger.error("Unauthorized access: No user_id in session")
        flash("Please sign in to view loans.", "error")
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    try:
        loans = supabase.table('loans').select('*').eq('user_id', session['user_id']).execute().data
        return jsonify({'loans': loans or []}), 200
    except Exception as e:
