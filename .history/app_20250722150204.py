...
def add_loan():
    try:
        ...
        age = int(form['age'])
        income = float(form['monthlyIncome'])
        ...
        user_id = "user-id-from-session-or-auth"  # Get user_id from session/token

        unique_id = str(uuid.uuid4())

        # Upload Aadhar
        aadhar_filename = f"aadhar/{user_id}_{unique_id}.pdf"
        supabase.storage.from_("aadhar").upload(aadhar_filename, aadhar_file.stream, content_type="application/pdf")

        # Upload PAN
        pan_filename = f"pan/{user_id}_{unique_id}.pdf"
        supabase.storage.from_("pan").upload(pan_filename, pan_file.stream, content_type="application/pdf")
        ...
        response = supabase.table("loans").insert({
            ...
            "aadhar_file": aadhar_filename,  # MUST be string, not list
            "pan_number": pan_number,
            "pan_file": pan_filename         # MUST be string
        }).execute()
