...
def confirm_loan():
    ...
    try:
        ...
        if not loan_id or not confirmation_text or not manager_name:
            logger.error("Missing required fields: loan_id, confirmation_text, or manager_name")
            return jsonify({'error': 'All fields are required'}), 400

        if confirmation_text.lower() != 'confirm loan':
            logger.error("Invalid confirmation text")
            return jsonify({'error': 'Invalid confirmation text. Please type "confirm loan".'}), 400
        ...
        logger.error(f"Error confirming loan: {str(e)}")
