...
def get_user_loans():
    ...
    logger.info(f"[get-user-loans] Session: {dict(session)}")

    # ✅ Check session keys
    if 'user_id' not in session or session.get('sign_in_as') != 'loan_borrower':
        logger.error("[get-user-loans] Unauthorized access")
        return jsonify({'error': 'Unauthorized'}), 401

    try:
        user_id = session['user_id']
        logger.info(f"[get-user-loans] Fetching loans for user ID: {user_id}")

        # ✅ Query Supabase for loans linked to this user
        response = supabase.table('loans').select('id, loan_amount, loan_purpose, status').eq('user_id', user_id).execute()

        if response.error:
            logger.error(f"[get-user-loans] Supabase error: {response.error}")
        ...
        for loan in loans:
            loan_data.append({
                ...
                'status': loan['status'].capitalize() if loan.get('status') else 'Pending'
            })

        logger.info(f"[get-user-loans] Returned {len(loan_data)} loans for user {user_id}")
        return jsonify({'loans': loan_data}), 200

    except Exception as e:
        logger.error(f"[get-user-loans] Exception: {str(e)}")
