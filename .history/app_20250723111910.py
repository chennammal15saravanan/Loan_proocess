...
def add_loan():
    try:
        ...
        bucket_name = 'loan-files'

        aadhar_filename = f"aadhar_{uuid.uuid4()}.pdf"
        pan_filename = f"pan_{uuid.uuid4()}.pdf"

        # Upload Aadhar
        aadhar_response = supabase.storage.from_(bucket_name).upload(
            aadhar_filename,
            aadhar_file.stream,
            file_options={"content-type": "application/pdf"}
        )
        if 'error' in aadhar_response:
            return jsonify({'error': 'Aadhar upload failed'}), 500

        # Upload PAN
        pan_response = supabase.storage.from_(bucket_name).upload(
            pan_filename,
            pan_file.stream,
            file_options={"content-type": "application/pdf"}
        )
        if 'error' in pan_response:
            return jsonify({'error': 'PAN upload failed'}), 500
