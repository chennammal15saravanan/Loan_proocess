...
def get_all_products():
    ...
    try:
        # Join products with user_profiles to get merchant username
        products = supabase.table('products').select('products.*, user_profiles.username').join('user_profiles', 'products.user_id = user_profiles.id').execute().data
        product_data = []
        for product in products:
            product_data.append({
                ...
                'price': product['price'],
                'merchant_name': product['username']  # From joined user_profiles
            })
...
def add_loan():
    if 'user_id' not in session:get_
        logger.error("Unauthorized access: No user_id in session")
