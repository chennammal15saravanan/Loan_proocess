...
def add_loan():
    ...
    try:
        ...
        if not all([first_name, last_name, dob, phone, address, age, monthly_income, occupation, loan_amount, loan_purpose, aadhar_number, pan_number]):
        ...
        try:
            existing_aadhar = supabase.rpc('custom_query', {'sql_text': query}).execute()
        ...
        try:
            ...
            if response.data:
                logger.info(f"Loan application submitted successfully for user {session.get('user_id')}")
...
def merchant_profile():
    ...
    try:
        ...
        merchant_id = f"MERCH{user_data['id'][:8]}"
    except Exception as e:
...
def loan_profiles():
    ...
    try:
        user_data = supabase.table('user_profiles').select('*').eq('id', session['user_id']).execute().data[0]
        loan_browser_id = f"LB{session['user_id'][:8]}"
    except Exception as e:
        flash(f"Error fetching profile data: {str(e)}", "error")
        return redirect(url_for('dashboard'))
