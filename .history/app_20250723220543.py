...
def add_loan():
    ...
    try:
        ...
        if aadhar_check.data:
            ...
            return jsonify({'error': 'Aadhar number already used'}),lood4
        ...
        loan_data = {
            ...
            'created_at': datetime.utcnow().isoformat(),
            'referred_by': None  # Add referred_by field, set to None if not provided
        }
        ...
        if 'row-level security policy' in str(e).lower():
            ...
            return jsonify({'error': 'Database configuration error: RLS policy violation. Please ensure RLS policies are correctly configured.'}), 400
        return jsonify({'error': str(e)}), 500
