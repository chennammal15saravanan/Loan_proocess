...
def update_loan_status():
    ...
    try:
        ...
        update_data = {
            'status': status.lower(),
            'review_status': 'confirmed' if status.lower() == 'approved' else 'rejected' if status.lower() == 'rejected' else 'awaiting',
            'manager_name': manager_name or session.get('username', 'Unknown'),
            'confirmed_at': datetime.utcnow().isoformat() if status.lower() == 'approved' else None
        }
...
def add_loan_documents(id):
    ...
    try:
        ...
        if not loan.data or loan.data[0]['status'].lower() != 'approved':
            logger.error(f"Loan {id} not found or not approved")
            return jsonify({'error': 'Only approved loans can add documents'}), 403
        ...
        for key, file in files.items():
            if file and file.filename:
                ...
                if file.content_length > 10 * 1024 * 1024:  # Updated to 10MB
                    logger.error(f"File size exceeds 10MB for {key}")
                    return jsonify({'error': f'File size for {key} must be less than 10MB.'}), 400
