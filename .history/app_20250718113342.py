...
def signup():
    if request.method == 'POST':
        ...
        try:
            ...
            # Signup with Supabase Auth (triggers confirmation email if enabled)
            ...
            if not auth_response.user:
                return render_template('sign-up.html', error="Signup failed. User not returned.")

            # Note: Session will be None until user confirms email
            user = auth_response.user

            # Save profile in DB with hashed password (don't allow login until confirmation)
            ...
            flash("✅ Signup successful! Please check your email to confirm before logging in.", "info")
