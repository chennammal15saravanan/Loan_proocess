    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not profile_response.data:
                logger.error("Failed to insert user profile into users table.")
            ...
            logger.info(f"User {username} signed up successfully with role {sign_up_as}.")
...
def signin():
    if request.method == 'POST':
        ...
        try:
            ...
            logger.info(f"User {profile.data[0]['username']} signed in with role {sign_in_as}.")
            return redirect(url_for('dashboard'))
