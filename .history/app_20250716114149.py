...
def signup():
    if request.method == 'POST':
        ...
        try:
            ...
            # Fallback: Sign in to get a session if none is provided
            access_token = auth_response.session.access_token if auth_response.session else None
            if not access_token:
                logger.warning(f"No access token from signup for {email}. Attempting sign-in.")
                ...
                logger.info(f"Sign-in successful, access_token obtained for user ID: {user.id}")
