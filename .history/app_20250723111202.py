...
def add_loan():
    ...
    try:
        ...
        # Optional: Save files to disk or cloud
        # For example:
        aadhar_path = f"uploads/aadhar_{uuid.uuid4()}.pdf"
        pan_path = f"uploads/pan_{uuid.uuid4()}.pdf"
        ...
        if response.data:
            logger.info(f"Loan application submitted by user {user_id}")
        ...
        logger.error(f"Error in add-loan: {str(e)}")
        return jsonify({'error': str(e)}), 500
