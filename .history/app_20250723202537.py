...
def signup():
    if request.method == 'POST':
        ...
        # Validate sign_up_as
        # Update to match HTML form values
        ...
        # Validate age (optional, but if provided, should be a number >= 18)
        if age:
            try:
                age = int(age)
                if age < 18:
            ...
            age = None  # Set to None if not provided, as per schema

        try:
            # Check if username exists
            if supabase.table('user_profiles').select('username').eq('username', username).execute().data:
                flash("Username already taken.", "error")
                logger.error(f"Username already taken: {username}")
                return render_template('sign-up.html')

            # Check if email exists
            if supabase.table('user_profiles').select('email').eq('email', email).execute().data:
            ...
            user_data = {
                ...
                'phone': phone or None,  # Store None if empty
                'age': age,  # Already validated as int or None
            ...
            if response.data:
                ...
                session['sign_in_as'] = sign_up_as  # Use sign_up_as for consistency
...
def add_loan():
    ...
    dob = request.form.get('dob')  # Expecting YYYY-MM-DD
    age = request.form.get('age')
    phone = request.form.get('phone')
    ...
    if not all([first_name, last_name, dob, age, phone, address, occupation,
                ...
                aadhar_number, pan_number, aadhar_file, pan_file]):
        return jsonify({'error': 'All fields are required'}), 400

    try:
        # Upload files to Supabase Storage
        aadhaar_path = f'aadhar/{user_id}_{secure_filename(aadhar_file.filename)}'
        pan_path = f'pan/{user_id}_{secure_filename(pan_file.filename)}'

        supabase.storage.from_('documents').upload(aadhaar_path, aadhar_file, file_options={"content-type": "application/pdf"})
        supabase.storage.from_('documents').upload(pan_path, pan_file, file_options={"content-type": "application/pdf"})
        ...
        if response.data:
            return jsonify({'message': 'Loan submitted', 'loan': response.data[0]}), 200
        else:
