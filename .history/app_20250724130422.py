...
def add_referral_loan():
    if 'user_id' not in session or session.get('sign_in_as') != 'merchant':
        logger.error("Unauthorized access: No user_id in session or not a merchant")
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    try:
        ...
        # Check if customer exists in user_profiles, if not create a new user
        user_check = supabase.table('user_profiles').select('id').eq('aadhaar_number', aadhaar_number).execute()
        if user_check.data:
            ...
            user_data = {
                'id': user_id,
                'username': f"{first_name.lower()}_{last_name.lower()}_{uuid.uuid4().hex[:8]}",
                'email': f"{first_name.lower()}.{last_name.lower()}@example.com",
                'password': generate_password_hash(str(uuid.uuid4())),  # Temporary password
                'sign_up_as': 'loan_borrower',
                'phone': phone,
                'age': age,
                'aadhaar_number': aadhaar_number,
                'pan_number': pan_number,
        ...
        try:
            response = supabase.table("loans").insert(loan_data).execute()
            if response.data:
                logger.info(f"Referral loan application submitted successfully by merchant {merchant_id}")
                ...
                supabase.storage.from_("documents").remove([aadhaar_filename, pan_filename])
                return jsonify({'error': 'Failed to save referral loan data.'}), 500
        except Exception as e:
            logger.error(f"Error inserting referral loan data: {str(e)}")
            supabase.storage.from_("documents").remove([aadhaar_filename, pan_filename])
            return jsonify({'error': f"Failed to save referral loan data: {str(e)}"}), 500
...
def get_loans():
    if 'user_id' not in session or session.get('sign_in_as') != 'merchant':
        logger.error("Unauthorized access to get-loans")
        return jsonify({'error': 'Unauthorized'}), 401

    try:
        merchant_id = session['user_id']
        loans = supabase.table('loans').select('id, first_name, last_name, loan_amount, status, aadhaar_url, pan_url, referred_by').eq('user_id', merchant_id).execute().data
        referred_loans = supabase.table('loans').select('id, first_name, last_name, loan_amount, status, aadhaar_url, pan_url, referred_by').eq('referred_by', merchant_id).execute().data
        all_loans = loans + referred_loans
        return jsonify({'loans': all_loans or []}), 200
    except Exception as e:
...
def get_referral_loans():
    ...
    try:
        merchant_id = session['user_id']
        loans = supabase.table('loans').select('id, first_name, last_name, loan_amount, status, aadhaar_url, pan_url').eq('referred_by', merchant_id).execute().data
        return jsonify({'referralLoans': loans or []}), 200
    except Exception as e:
...
def get_loan_applications():
    ...
    try:
        loans = supabase.table('loans').select('id, user_id, first_name, last_name, loan_amount, status, aadhaar_url, pan_url, referred_by, created_at').execute().data
        loan_data = []
        for loan in loans:
            user = supabase.table('user_profiles').select('username').eq('id', loan['user_id']).execute().data
            referrer = supabase.table('user_profiles').select('username').eq('id', loan['referred_by']).execute().data if loan['referred_by'] else None
            loan_data.append({
                'id': loan['id'],
                ...
                'referred_by': referrer[0]['username'] if referrer else 'Direct',
...
@app.route('/update-loan-status/<loan_id>', methods=['POST'])
