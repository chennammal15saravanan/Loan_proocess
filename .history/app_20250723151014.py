...
def signup():
    if request.method == 'POST':
        ...
        try:
            # Use RPC to check for existing username or email
            response = supabase.rpc('check_user_exists', {'p_username': username, 'p_email': email}).execute()
            if response.data:
...
def add_loan():
    ...
    try:
        ...
        response = supabase.rpc('check_user_exists', {'p_aadhar_number': aadhar_number}).execute()
        if response.data:
