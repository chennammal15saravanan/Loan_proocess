...
def add_product():
    ...
    try:
        ...
        logger.info(f"Adding product for user {user_id}: {product_data}")
        response = supabase.table('products').insert(product_data).execute()

        if response.data:
            logger.info(f"Product {product_data['product_id']} added successfully")
            flash("Product added successfully!", "info")
            return jsonify({'message': 'Product added successfully', 'product': response.data[0]}), 200
        else:
            logger.error("Failed to add product: No data returned")
            flash("Failed to add product.", "error")
            return jsonify({'error': 'Failed to add product'}), 500

    except ValueError:
        logger.error("Invalid price: not a number")
...
def update_product(product_id):
    ...
    try:
        ...
        if not product.data:
            logger.error(f"Product {product_id} not found or not owned by user {user_id}")
        ...
        logger.info(f"Updating product {product_id} with data: {product_data}")
        ...
        if response.data:
            ...
            logger.error(f"Failed to update product {product_id}: No data returned")
