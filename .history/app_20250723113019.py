...
def add_loan():
    try:
        ...
        # Get uploaded files
        aadhar_file = request.files["aadharFile"]
        ...
        # Define bucket and filenames
        bucket = "loan-files"
        aadhar_filename = f"aadhar_{uuid.uuid4()}.pdf"
        pan_filename = f"pan_{uuid.uuid4()}.pdf"

        # Upload Aadhar file
        supabase.storage.from_(bucket).upload(
            path=aadhar_filename,
            file=aadhar_file.stream,
            file_options={"content-type": aadhar_file.mimetype}
        )

        # Upload PAN file
        supabase.storage.from_(bucket).upload(
            path=pan_filename,
            file=pan_file.stream,
            file_options={"content-type": pan_file.mimetype}
        )

        # Get file public URLs (optional)
        aadhar_url = supabase.storage.from_(bucket).get_public_url(aadhar_filename)
        pan_url = supabase.storage.from_(bucket).get_public_url(pan_filename)

        # Insert into database
        supabase.table("loans").insert({
            "user_id": session.get("user_id"),  # assumes you use session for login
            "full_name": full_name,
            "age": age,
            ...
            "aadhar_file_path": aadhar_filename,
            "pan_file_path": pan_filename,
        ...
        print("Error in add_loan:", e)
        flash("Something went wrong while submitting your application.", "error")
        return redirect(url_for("dashboard"))
