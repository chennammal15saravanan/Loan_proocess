...
def get_all_products():
    try:
        # Join products with merchant user table (optional, if you want merchant names)
        products = supabase.table('products').select('name, description, price, user_id').execute().data

        enriched_products = []
        for p in products:
            merchant_info = supabase.table('user_profiles').select('full_name').eq('user_id', p['user_id']).execute().data
            enriched_products.append({
                'name': p['name'],
                'description': p.get('description', ''),
                'price': p['price'],
                'merchant_name': merchant_info[0]['full_name'] if merchant_info else 'Unknown'
            })
