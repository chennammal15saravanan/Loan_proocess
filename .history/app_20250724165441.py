...
def signin():
    # [Existing signin logic, unchanged]
    if request.method == 'POST':
        email = request.form.get('email')
        password = request.form.get('password')
...
def dashboard():
    ...
    if role == 'merchant':
        ...
        try:
            # Fetch all products for loan borrower
            products = supabase.table('products').select('*').execute().data
            return render_template('loan-browser.html', username=username, role=role, products=products or [])
        except Exception as e:
            logger.error(f"Error fetching products for loan borrower: {str(e)}")
            flash(f"Error fetching products: {str(e)}", "error")
            return render_template('loan-browser.html', username=username, role=role, products=[])
...
# Add Referral Loan for Merchants (unchanged)
...
# Get Referral Loans for Merchants (unchanged)
...
def get_referral_loans():
    ...
    try:
        loans = supabase.table('loans').select('id, first_name, last_name, loan_amount, status, created_at, aadhaar_url, pan_url, referred_by').eq('referred_by', session['user_id']).execute().data
        loan_data = []
        for loan in loans:
            loan_data.append({
                'request_id': loan['id'],
                ...
                'aadhaar_url': loan['aadhaar_url'],
                'pan_url': loan['pan_url'],
...
# Get Loans for Loan Borrowers (unchanged)
...
def get_user_loans():
    ...
    try:
        loans = supabase.table('loans').select('id, loan_amount, loan_purpose, status').eq('user_id', session['user_id']).execute().data
        loan_data = []
        for loan in loans:
            loan_data.append({
                'loan_id': loan['id'],
        ...
        logger.info(f"Fetched {len(loan_data)} loans for user {session['user_id']}")
        return jsonify({'loans': loan_data}), 200
    except Exception as e:
...
# Get All Products for Loan Borrowers
@app.route('/get-all-products', methods=['GET'])
def get_all_products():
    if 'user_id' not in session or session.get('sign_in_as') != 'loan_borrower':
        logger.error("Unauthorized access to get-all-products")
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    try:
        ...
        logger.info(f"Fetched {len(product_data)} products for loan borrower {session['user_id']}")
...
# Updated Get Loan Applications for NBFC Admin
...
def get_loan_applications():
    ...
    try:
        # Fetch all loans
        loans = supabase.table('loans').select('id, user_id, first_name, last_name, loan_amount, status, created_at, referred_by, address').execute().data
        customer_loans = []
        referral_loans = []
        
        for loan in loans:
            user = supabase.table('user_profiles').select('username').eq('id', loan['user_id']).execute().data
            ...
            # Separate loans based on referred_by
            if loan['referred_by'] and loan['referred_by'] == loan['user_id']:
                referrer = supabase.table('user_profiles').select('username').eq('id', loan['referred_by']).execute().data
                loan_entry['referred_by'] = referrer[0]['username'] if referrer else 'Unknown'
                referral_loans.append(loan_entry)
            else:
                customer_loans.append(loan_entry)
                
        logger.info(f"Fetched {len(customer_loans)} customer loans and {len(referral_loans)} referral loans")
        return jsonify({
            'customer_loans': customer_loans,
            'referral_loans': referral_loans
        }), 200
    except Exception as e:
        logger.error(f"Error fetching loan applications: {str(e)}")
        return jsonify({'error': str(e)}), 500

# Updated Update Loan Status for NBFC Admin
...
# Get Products for Merchants (unchanged)
