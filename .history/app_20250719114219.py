...
def dashboard():
    ...
    role = session['sign_in_as']
    if role == 'merchant':
        return redirect(url_for('product_desc'))
    elif role == 'loan_borrower':
        return redirect(url_for('view_loan_request'))
...
@app.route('/view-loan-request')
def view_loan_request():
    if 'user_id' not in session:
        flash("Please sign in to continue.", "error")
        return redirect(url_for('signin'))
    return render_template('view-loan-request.html', username=session['username'])
...
def product_desc():
    if 'user_id' not in session:
        flash("Please sign in to continue.", "error")
        return redirect(url_for('signin'))
    return render_template('product-desc.html', username=session['username'])
...
def account_setting():
    if 'user_id' not in session:
        flash("Please sign in to continue.", "error")
        return redirect(url_for('signin'))
    return render_template('account-setting.html', username=session['username'])
