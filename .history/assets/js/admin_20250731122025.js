    ...
    .then(data => {
      ...
      data.loans.forEach(loan => {
        ...
        row.innerHTML = `
          ...
          <td>₹${parseFloat(loan.amount || 0).toFixed(2)}</td> <!-- Use loan.amount to match backend key -->
...
function viewLoanDetails(loanId) {
  fetch(`/get-loan-details/${loanId}`)
    ...
    .then(data => {
      ...
      const hasDocuments = loan.appraisal_slip_url || loan.invoice_url || loan.gold_photo_url || 
                          loan.aadhaar_front_url || loan.aadhaar_back_url || loan.pan_front_url || 
                          loan.pan_back_url || loan.bank_statement_url || loan.income_proof_url || 
                          loan.address_proof_url || loan.utility_bill_url;
            ...
            <div class="modal-footer d-flex justify-content-between">
              <div>
                ${loan.status.toLowerCase() === 'pending' ?
                  ...
                  ''}
                ${hasDocuments && loan.status.toLowerCase() === 'accepted' ?
                  `<button type="button" class="btn btn-info me-2" onclick="viewLoanDocuments('${loanId}')">View Documents</button>` : ''}
              </div>
              <div>
...
function viewLoanDocuments(loanId) {
  fetch(`/get-loan-details/${loanId}`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
    ...
    .then(data => {
      ...
      const documents = [
        { name: 'Appraisal Slip', url: loan.appraisal_slip_url },
        { name: 'Invoice', url: loan.invoice_url },
        { name: 'Gold Product Live Photo', url: loan.gold_photo_url },
        { name: 'Aadhaar Front', url: loan.aadhaar_front_url },
        { name: 'Aadhaar Back', url: loan.aadhaar_back_url },
        { name: 'PAN Front', url: loan.pan_front_url },
        { name: 'PAN Back', url: loan.pan_back_url },
        { name: '6-Month Bank Statement', url: loan.bank_statement_url },
        { name: 'Income Proof', url: loan.income_proof_url },
        { name: 'Address Proof', url: loan.address_proof_url },
        { name: 'Utility Bill', url: loan.utility_bill_url }
      ];
      ...
      documents.forEach(doc => {
        if (doc.url) {
          ...
          docElement.innerHTML = `
            <p><strong>${doc.name}:</strong> <a href="${doc.url}" class="text-primary text-decoration-underline" target="_blank">View</a> | <a href="${doc.url}" class="text-primary text-decoration-underline" download>Download</a></p>
          `;
      ...
      const modal = new bootstrap.Modal(document.getElementById('viewDocumentsModal'));
      modal.show();
    })
    .catch(error => {
      console.error('Error fetching loan documents:', error);
      showToast(`Error fetching loan documents: ${error.message}. Please try again.`, true);
    });
}
