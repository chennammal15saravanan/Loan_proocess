...
function reviewDocuments(loanId) {
  fetch(`/get-referral-loan-details/${loanId}`)
    ...
    .then(data => {
      ...
      const loan = data.loan;
      const documentUrls = [
        loan.appraisal_slip_url,
        loan.invoice_url,
        loan.gold_photo_url,
        loan.aadhaar_front_url,
        loan.aadhaar_back_url,
        loan.pan_front_url,
        loan.pan_back_url,
        loan.bank_statement_url,
        loan.income_proof_url,
        loan.address_proof_url,
        loan.utility_bill_url
      ].filter(url => url !== 'N/A');

      if (documentUrls.length > 0) {
        documentUrls.forEach(url => {
          const link = document.createElement('a');
          link.href = `/download-document/${encodeURIComponent(url)}`;
          link.download = ''; // Triggers download
          link.target = '_blank'; // Opens in new tab for viewing
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
        });
        showToast('Documents are being downloaded.', false);
      } else {
    ...
    .catch(error => {
      ...
      showToast(`Error reviewing documents: ${error.message}. Please try again.`, true);
    });
}
