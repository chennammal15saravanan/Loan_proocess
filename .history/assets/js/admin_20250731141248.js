    ...
    .then(data => {
      ...
      let merchantDetails = `
        <div class="symmetric-col mt-4">
          ...
          ${loan.status.toLowerCase() === 'accepted' && (loan.appraisal_slip_url !== 'N/A' || loan.invoice_url !== 'N/A' || loan.gold_photo_url !== 'N/A' || loan.aadhaar_front_url !== 'N/A' || loan.aadhaar_back_url !== 'N/A' || loan.pan_front_url !== 'N/A' || loan.pan_back_url !== 'N/A' || loan.bank_statement_url !== 'N/A' || loan.income_proof_url !== 'N/A' || loan.address_proof_url !== 'N/A' || loan.utility_bill_url !== 'N/A') ? `
            <p><button type="button" class="btn btn-pink" onclick="reviewDocuments('${loanId}')">Review Document</button></p>
          ` : ''}
        </div>
      `;
...
function reviewDocuments(loanId) {
  fetch(`/get-referral-loan-details/${loanId}`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
    ...
    .then(data => {
      ...
      const documentUrls = [
        ...
        { name: 'Gold Photo', url: loan.gold_photo_url },
        ...
        { name: 'Bank Statement', url: loan.bank_statement_url },
      ...
      ].filter(doc => doc.url !== 'N/A');

      if (documentUrls.length === 0) {
        showToast('No documents available for review.', true);
        return;
      }
              ...
              <ul class="list-group">
                ${documentUrls.map(doc => `
                  <li class="list-group-item d-flex justify-content-between align-items-center">
                    <span>${doc.name}</span>
                    <div>
                      <a href="${doc.url}" class="btn btn-primary btn-sm me-2" target="_blank">View</a>
                      <a href="/download-document/${encodeURIComponent(doc.url)}" class="btn btn-success btn-sm" download>Download</a>
                    </div>
                  </li>
                `).join('')}
              </ul>
