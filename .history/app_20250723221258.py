...
def add_loan():
    ...
    try:
        # Get form data matching HTML field names and schema
        ...
        dob = request.form.get("dob")  # New: Date of birth
        phone = request.form.get("phone")  # New: Phone number
        address = request.form.get("address")  # New: Address
        user_id = session.get("user_id")
        referred_by = request.form.get("referredBy")  # Optional

        # Validate required fields
        required_fields = [full_name, age, monthly_income, occupation, loan_amount, 
                         loan_purpose, aadhar_number, pan_number, dob, phone, address]
        ...
        if len(name_parts) < 1:
            logger.error("Invalid full name: Must provide at least a first name")
            return jsonify({'error': 'Full name must include at least a first name.'}), 400
        ...
        # Check for existing Aadhar number (UNIQUE constraint)
        try:
            existing_aadhar = supabase.table('loans').select('aadhar_number').eq('aadhar_number', aadhar_number).execute()
            if existing_aadhar.data:
                logger.error(f"Aadhar number {aadhar_number} already exists")
                return jsonify({'error': 'Aadhar number already exists.'}), 400
        except Exception as e:
            logger.error(f"Error checking Aadhar number: {str(e)}")
            return jsonify({'error': 'Internal error checking Aadhar number.'}), 500

        # Check for existing PAN number (UNIQUE constraint)
        try:
            ...
            logger.error(f"Error checking PAN number: {str(e)}")
            return jsonify({'error': 'Internal error checking PAN number.'}), 500

        # Validate referred_by if provided
        if referred_by:
            try:
                referrer = supabase.table('user_profiles').select('id').eq('id', referred_by).execute()
                if not referrer.data:
                    logger.error(f"Invalid referred_by ID: {referred_by}")
                    return jsonify({'error': 'Invalid referrer ID.'}), 400
            except Exception as e:
                logger.error(f"Error validating referred_by: {str(e)}")
                return jsonify({'error': 'Internal error validating referrer.'}), 500
        ...
        aadhar_filename = f"aadhar/{uuid.uuid4()}_{secure_filename(aadhar_file.filename)}"
        ...
        aadhar_file_url = supabase.storage.from_("documents").get_public_url(aadhar_filename)
        ...
        try:
            ...
            if response.data:
                ...
                supabase.storage.from_("documents").remove([aadhar_filename, pan_filename])
