...
def get_referral_loans():
    ...
    try:
        ...
        return jsonify({'loans': loan_data}), 200  # Fixed: Changed referral_loans.data to loan_data
    except Exception as e:
...
def get_loan_applications():
    ...
    try:
        loans = supabase.table('loans').select('id, first_name, last_name, address, loan_amount, status, created_at, referred_by').execute().data
        loan_data = []
        for loan in loans:
            referred_by = loan.get('referred_by')
            merchant_name = 'N/A'
            if referred_by:
                user = supabase.table('user_profiles').select('username').eq('id', referred_by).execute().data
...
def update_loan_status():
    ...
    try:
        ...
        loan = supabase.table('loans').select('*').eq('id', request_id).execute()  # Fixed: Changed 'request_id' to 'id'
        if not loan.data:
            logger.error(f"Loan not found: {request_id}")
            return jsonify({'error': 'Loan not found'}), 404
        response = supabase.table('loans').update({'status': status.lower()}).eq('id', request_id).execute()  # Fixed: Changed 'request_id' to 'id'
        if response.data:
