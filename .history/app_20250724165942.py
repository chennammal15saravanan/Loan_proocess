...
def get_referral_loans():
    ...
    try:
        ...
        return jsonify({'loans': referral_loans.data})
...
def add_product():
    ...
    try:
        product_data = {
            ...
            'price': float(price)
            # Removed 'created_at' since it's not in the schema
        }
...
def get_loan_applications():
    ...
    try:
        loans = supabase.table('loans').select(' user_id, first_name, last_name, loan_amount, status, created_at').execute().data
        loan_data = []
        for loan in loans:
            user = supabase.table('user_profiles').select('username').eq('id', loan['user_id']).execute().data
            merchant_name = user[0]['username'] if user else 'Unknown'
            loan_data.append({
                'first_name': loan['user_id'] if loan.get('user_id') else str(uuid.uuid4()),
...
def update_loan_status():
    ...
    try:
        ...
        if not request_id or not status:
            logger.error("Missing request_id or status")
            return jsonify({'error': 'Request ID and status are required'}), 400
        if status not in ['Accepted', 'Rejected']:
            logger.error(f"Invalid status: {status}")
            return jsonify({'error': 'Invalid status'}), 400
        loan = supabase.table('loans').select('*').eq('request_id', request_id).execute()
        if not loan.data:
            logger.error(f"Loan not found: {request_id}")
            return jsonify({'error': 'Loan not found'}), 404
        response = supabase.table('loans').update({'status': status.lower()}).eq('request_id', request_id).execute()
        if response.data:
            logger.info(f"Loan {request_id} status updated to {status}")
            return jsonify({'message': f'Loan {status} successfully'}), 200
        else:
            logger.error(f"Failed to update loan status for {request_id}")
