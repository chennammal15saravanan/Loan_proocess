    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if check_password_hash(user_data['password'], password):
                ...
                session['sign_in_as'] = user_data['sign_up_as']  # Fetch role from DB
                logger.info(f"User signed in: {user_data['username']}, role: {user_data['sign_up_as']}")
...
@app.route('/get-referral-loan-details/<id>', methods=['GET'])
def get_referral_loan_details(id):
    if 'user_id' not in session or session.get('sign_in_as') != 'merchant':
        logger.error("Unauthorized access to get-referral-loan-details")
        return jsonify({'error': 'Unauthorized'}), 401

    try:
        loan = supabase.table('loans').select('*').eq('id', id).eq('referred_by', session['user_id']).execute()
        if not loan.data:
            logger.error(f"Loan not found or unauthorized: {id}")
            return jsonify({'error': 'Loan not found or unauthorized'}), 404

        loan_data = loan.data[0]
        # Include document URLs only if status is 'accepted'
        response_data = {
            ...
            'monthly_income': float(loan_data['monthly_income']) if loan_data.get('monthly_income') else 0.0,
            'loan_amount': float(loan_data['loan_amount']) if loan_data.get('loan_amount') else 0.0,
            ...
            'appraisal_slip_url': loan_data.get('appraisal_slip_url', 'N/A') if loan_data.get('status', '').lower() == 'accepted' else 'N/A',
            'invoice_url': loan_data.get('invoice_url', 'N/A') if loan_data.get('status', '').lower() == 'accepted' else 'N/A',
            'gold_photo_url': loan_data.get('gold_photo_url', 'N/A') if loan_data.get('status', '').lower() == 'accepted' else 'N/A',
            'aadhaar_front_url': loan_data.get('aadhaar_front_url', 'N/A') if loan_data.get('status', '').lower() == 'accepted' else 'N/A',
            'aadhaar_back_url': loan_data.get('aadhaar_back_url', 'N/A') if loan_data.get('status', '').lower() == 'accepted' else 'N/A',
            'pan_front_url': loan_data.get('pan_front_url', 'N/A') if loan_data.get('status', '').lower() == 'accepted' else 'N/A',
            'pan_back_url': loan_data.get('pan_back_url', 'N/A') if loan_data.get('status', '').lower() == 'accepted' else 'N/A',
            'bank_statement_url': loan_data.get('bank_statement_url', 'N/A') if loan_data.get('status', '').lower() == 'accepted' else 'N/A',
            'income_proof_url': loan_data.get('income_proof_url', 'N/A') if loan_data.get('status', '').lower() == 'accepted' else 'N/A',
            'address_proof_url': loan_data.get('address_proof_url', 'N/A') if loan_data.get('status', '').lower() == 'accepted' else 'N/A',
            'utility_bill_url': loan_data.get('utility_bill_url', 'N/A') if loan_data.get('status', '').lower() == 'accepted' else 'N/A'
        }
...
@app.route('/download-document/<path:url>', methods=['GET'])
def download_document(url):
    try:
        # Ensure the URL is a valid Supabase public URL
        if not url.startswith('https://') or 'supabase.co/storage' not in url:
            logger.error(f"Invalid document URL: {url}")
            return jsonify({'error': 'Invalid document URL'}), 400

        # Redirect to the public URL to trigger download
