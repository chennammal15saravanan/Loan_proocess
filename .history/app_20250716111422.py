...
def signup():
    if request.method == 'POST':
        ...
        if not email or not username or not password:
            logger.error("Missing input in signup form")
        ...
        if not is_valid_password(password):
            ...
            return render_template('sign-up.html', error="Password must be at least 8 characters long.")
        ...
        try:
            ...
            if not user or not session_data:
                logger.error("Supabase Auth failed: No user or session returned")
                return render_template('sign-up.html', error="Signup failed. Check your email confirmation.")
            ...
            # ✅ Step 3: Insert profile using access_token for RLS
            logger.info(f"Inserting user profile into 'profiles' table using authenticated session")
            ...
            if insert_response.data:
                logger.info("Signup successful, redirecting to sign-in")
                return redirect(url_for('signin'))
            else:
                logger.error("Failed to insert user profile to database")
    ...
    # GET method - render the sign-up page
    return render_template('sign-up.html')
