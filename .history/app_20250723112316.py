...
def add_loan():
    try:
        ...
        user_id = session.get('user_id')  # Make sure session is set up

        aadhar_file = request.files.get('aadhar')
        pan_file = request.files.get('pan')

        if not all([name, contact, loan_amount, aadhar_file, pan_file]):
            flash("All fields are required.", "error")
            return redirect(request.url)

        # Generate unique filenames
        aadhar_filename = f"{uuid.uuid4()}_{aadhar_file.filename}"
        pan_filename = f"{uuid.uuid4()}_{pan_file.filename}"
