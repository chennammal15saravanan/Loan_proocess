...
def signup():
    if request.method == 'POST':
        ...
        try:
            ...
            if not access_token:
                logger.error(f"Signup succeeded for {email}, but no access token returned.")
                return render_template('sign-up.html', error="Signup failed. Unable to authenticate user.")
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not user or not access_token:
                logger.error(f"Sign-in failed for email {email}: No user or access token")
...
def debug_profiles():
    try:
        # Use service_role key for debugging (securely store this key)
