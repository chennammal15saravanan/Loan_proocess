...
def update_loan_status():
    ...
    try:
        ...
        if not loan_id or not status:
            logger.warning("Missing loan_id or status in form data")
        ...
        # Perform update
        result = supabase.table('loans').update({'status': status}).eq('id', loan_id).execute()

        if result.data:
            logger.info(f"Loan {loan_id} status updated to {status}")
            return jsonify({'message': f'Loan {status.capitalize()} successfully'}), 200
        else:
            logger.error(f"Loan status update failed for ID: {loan_id}")
            return jsonify({'error': 'Failed to update loan status'}), 500

    except Exception as e:
        logger.exception("Exception during loan status update")
        return jsonify({'error': str(e)}), 500
