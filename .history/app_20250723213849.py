...
def add_loan():
    ...
    # Validate required fields
    required_fields = [
        first_name, last_name, dob, age, phone, address, occupation,
    ...
    if aadhar_file and (not aadhar_file.filename.endswith('.pdf') or aadhar_file.content_length > max_file_size):
    ...
    if pan_file and (not pan_file.filename.endswith('.pdf') or pan_file.content_length > max_file_size):
    ...
    try:
        ...
        # Upload files to Supabase Storage 'documents' bucket
        bucket_name = 'documents'
        aadhar_filename = secure_filename(aadhar_file.filename)
        pan_filename = secure_filename(pan_file.filename)
        aadhar_path = f'aadhar/{user_id}_{aadhar_filename}'
        pan_path = f'pan/{user_id}_{pan_filename}'
        ...
        aadhar_upload = supabase.storage.from_(bucket_name).upload(
            path=aadhar_path,
            file=aadhar_file_content,
            file_options={"content-type": "application/pdf"}
        )
        pan_upload = supabase.storage.from_(bucket_name).upload(
            path=pan_path,
            file=pan_file_content,
            file_options={"content-type": "application/pdf"}
        )

        # Verify uploads
        if not (hasattr(aadhar_upload, 'status_code') and aadhar_upload.status_code == 200 and
                hasattr(pan_upload, 'status_code') and pan_upload.status_code == 200):
            logger.error(f"File upload failed: Aadhar status {getattr(aadhar_upload, 'status_code', 'unknown')}, "
                        f"PAN status {getattr(pan_upload, 'status_code', 'unknown')}")
            return jsonify({'error': 'Failed to upload files to storage'}), 500

        # Generate public URLs for files
        aadhar_url = supabase.storage.from_(bucket_name).get_public_url(aadhar_path)
        pan_url = supabase.storage.from_(bucket_name).get_public_url(pan_path)
        ...
        if response.data:
            logger.info(f"Loan submitted successfully for user_id: {user_id}, aadhar_url: {aadhar_url}, pan_url: {pan_url}")
        ...
        if 'row-level security policy' in str(e).lower():
            ...
            return jsonify({'error': 'Database configuration error: RLS policy violation. Please contact support.'}), 400
        if 'Bucket not found' in str(e):
            logger.error(f"Storage bucket '{bucket_name}' not found")
            return jsonify({'error': f"Storage bucket '{bucket_name}' not found. Please contact support."}), 500
        return jsonify({'error': str(e)}), 500
