...
@app.route('/confirm-loan', methods=['POST'])
def confirm_loan():
    if 'user_id' not in session or session.get('sign_in_as') != 'nbfc_admin':
        logger.error("Unauthorized access to confirm-loan")
        return jsonify({'error': 'Unauthorized: Please sign in as NBFC admin'}), 401
    ...
    try:
        ...
        if not loan_id or not confirmation_text or not manager_name:
            logger.error(f"Missing required fields: loan_id={loan_id}, confirmation_text={confirmation_text}, manager_name={manager_name}")
            return jsonify({'error': 'All fields (loan ID, confirmation text, and manager name) are required'}), 400

        if confirmation_text.lower() != 'confirm loan':
            logger.error(f"Invalid confirmation text: {confirmation_text}")
            return jsonify({'error': 'Please type "confirm loan" exactly'}), 400

        # Verify loan exists and is in a confirmable state
        loan = supabase.table('loans').select('id, status').eq('id', loan_id).execute()
        if not loan.data:
            logger.error(f"Loan not found: {loan_id}")
            return jsonify({'error': 'Loan not found'}), 404
        if loan.data[0]['status'] != 'pending':
            logger.error(f"Loan {loan_id} is not in pending status: {loan.data[0]['status']}")
            return jsonify({'error': 'Loan cannot be confirmed; it is not in pending status'}), 400
        ...
        if response.data:
            logger.info(f"Loan {loan_id} confirmed and status updated to accepted by {manager_name}")
            return jsonify({'message': 'Loan confirmed and status updated to accepted'}), 200
        else:
            logger.error(f"Failed to update loan status for {loan_id}")
            return jsonify({'error': 'Failed to update loan status'}), 500

    except Exception as e:
        logger.error(f"Error confirming loan {loan_id}: {str(e)}")
        return jsonify({'error': f'Internal server error: {str(e)}'}), 500

# Add Referral Loan for Merchants
...
def add_referral_loan():
    ...
    try:
        ...
        loan_data = {
            "user_id": user_id,  # Merchant's user_id
            ...
            "referred_by": user_id  # Merchant referring the loan
        }
...
# Get Referral Loans for Merchants
...
def get_all_products():
    try:
        # Get all products added by merchants
...
def add_loan():
    ...
    try:
        ...
        loan_amount = request.form.get("loanAmount")
        loan_purpose = request.form.get("loanPurpose")
        aadhaar_number = request.form.get("aadharNumber")  # Note: Keeping form field name as is
        pan_number = request.form.get("panNumber")
        user_id = session.get("user_id")
        ...
        loan_data = {
            ...
            "loan_amount": loan_amount,
            "loan_purpose": loan_purpose,
            "aadhaar_number": aadhaar_number,  # Corrected column name
            "pan_number": pan_number,
            "aadhaar_url": aadhaar_file_url,   # Corrected variable name
            "pan_url": pan_file_url,
            "status": "pending",
            "created_at": datetime.utcnow().isoformat(),
            "review_status": "awaiting"
...
def get_loan_details(id):
    ...
    try:
        ...
        # Fetch merchant details if referred_by exists
        merchant_name = 'N/A'
        ...
        # Prepare response with all loan details
        response_data = {
            'first_name': loan_data['first_name'],
            'last_name': loan_data['last_name'] or 'N/A',
            'dob': loan_data['dob'] or 'N/A',
            'age': loan_data['age'] or 'N/A',
            'phone': loan_data['phone'] or 'N/A',
            'address': loan_data['address'] or 'N/A',
            'occupation': loan_data['occupation'] or 'N/A',
            ...
            'loan_purpose': loan_data['loan_purpose'] or 'N/A',
            'aadhaar_number': loan_data['aadhaar_number'] or 'N/A',
            'pan_number': loan_data['pan_number'] or 'N/A',
            'aadhaar_url': loan_data['aadhaar_url'] or 'N/A',
            'pan_url': loan_data['pan_url'] or 'N/A',
            'status': loan_data['status'].capitalize() if loan_data['status'] else 'N/A',
            'created_at': loan_data['created_at'] or 'N/A',
            'review_status': loan_data['review_status'] or 'N/A',
...
def admin_profile():
    ...
    try:
        ...
        admin_id = user_data.get('admin_id', f"ADMIN{user_data['id'][:8]}")  # Generate admin_id if not stored
...
def get_loan_applications():
    ...
    try:
        loans = supabase.table('loans').select('id, first_name, address, loan_amount, status').execute().data
        loan_data = []
        for loan in loans:
            loan_data.append({
                'request_id': loan['id'],
...
def update_loan_status():
    ...
    try:
        loan_id = request.form.get('loan_id')  # Changed from request_id to loan_id
        ...
        if not loan.data:
            logger.error(f"Loan not found: {loan_id}")
            return jsonify({'error': 'Loan not found'}), 404
        response = supabase.table('loans').update({'status': status.lower()}).eq('id', loan_id).execute()
        if response.data:
