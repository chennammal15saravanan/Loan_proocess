...
def add_loan():
    try:
        ...
        if not all([full_name, age_raw, monthly_income, occupation, loan_amount, loan_purpose, aadhar_number, pan_number]):
            flash("All fields are required.", "error")
            return redirect("/loan-application-form")
        ...
        if not aadhar_file or not pan_file:
            flash("Both Aadhar and PAN files are required.", "error")
            return redirect("/loan-application-form")
        ...
        supabase.storage.from_("loan-files").upload(aadhar_filename, aadhar_file, {"content-type": aadhar_file.content_type})
        supabase.storage.from_("loan-files").upload(pan_filename, pan_file, {"content-type": pan_file.content_type})
