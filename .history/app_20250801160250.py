...
def update_product(product_id):
    ...
    try:
        ...
        response = supabase.table('products').update(product_data).eq('productid', product_id).execute()
    ...
    try:
        ...
        for key, file in files.items():
            if file and file.filename:
                ...
                # Map file keys to database column names
                column_name = {
                    'appraisal_slip': 'appraisal_slip_url',
                    'invoice': 'invoice_url',
                    'gold_photo': 'gold_photo_url',
        ...
        if update_data:
            logger.info(f"Updating loan {id} with data: {update_data}")
