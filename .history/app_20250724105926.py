...
def dashboard():
    ...
    if role == 'merchant':
        ...
        return render_template('nbfc_a.html', username=username, role=role)
    else:
...
def get_loan_applications():
    ...
    try:
        # Fetch all loan applications from the loans table
        loans = supabase.table('loans').select('request_id, user_id, first_name, last_name, loan_amount, status, created_at').execute().data

        # Fetch user data to get merchant_name (assuming username from user_profiles)
        loan_data = []
        for loan in loans:
            ...
            loan_data.append({
                'request_id': loan['request_id'] if loan.get('request_id') else str(uuid.uuid4()),  # Generate request_id if missing
