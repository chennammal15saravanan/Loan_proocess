    ...
    .then(data => {
      ...
      let merchantDetails = `
        <div class="symmetric-col mt-4">
          ...
          <p><button type="button" class="btn btn-pink" onclick="showDocumentModal('${loanId}')">Review Document</button></p>
        </div>
      `;
...
// New function to show document modal
function showDocumentModal(loanId) {
  fetch(`/get-loan-details/${loanId}`) // Adjust to /get-referral-loan-details if for merchant
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
    ...
    .then(data => {
      ...
      const loan = data.loan;
      console.log("Loan data in showDocumentModal:", loan); // Debug log
      ...
      documentModal.innerHTML = `
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">Loan Documents</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
              <div class="row g-3">
                <div class="col-12">
                  <h6 class="fw-semibold mb-3">Available Documents</h6>
                  <ul class="list-group">
                    ${generateDocumentItem('Aadhaar Document', loan.aadhaar_url, loan.aadhaar_url && loan.aadhaar_url.endsWith('.pdf') ? 'pdf' : 'image')}
                    ${generateDocumentItem('PAN Document', loan.pan_url, loan.pan_url && loan.pan_url.endsWith('.pdf') ? 'pdf' : 'image')}
                    ${generateDocumentItem('Appraisal Slip', loan.appraisal_slip_url, loan.appraisal_slip_url && loan.appraisal_slip_url.endsWith('.pdf') ? 'pdf' : 'image')}
                    ${generateDocumentItem('Invoice', loan.invoice_url, loan.invoice_url && loan.invoice_url.endsWith('.pdf') ? 'pdf' : 'image')}
                    ${generateDocumentItem('Gold Photo', loan.gold_photo_url, loan.gold_photo_url && loan.gold_photo_url.endsWith('.pdf') ? 'pdf' : 'image')}
                    ${generateDocumentItem('Aadhaar Front', loan.aadhaar_front_url, loan.aadhaar_front_url && loan.aadhaar_front_url.endsWith('.pdf') ? 'pdf' : 'image')}
                    ${generateDocumentItem('Aadhaar Back', loan.aadhaar_back_url, loan.aadhaar_back_url && loan.aadhaar_back_url.endsWith('.pdf') ? 'pdf' : 'image')}
                    ${generateDocumentItem('PAN Front', loan.pan_front_url, loan.pan_front_url && loan.pan_front_url.endsWith('.pdf') ? 'pdf' : 'image')}
                    ${generateDocumentItem('PAN Back', loan.pan_back_url, loan.pan_back_url && loan.pan_back_url.endsWith('.pdf') ? 'pdf' : 'image')}
                    ${generateDocumentItem('Bank Statement', loan.bank_statement_url, loan.bank_statement_url && loan.bank_statement_url.endsWith('.pdf') ? 'pdf' : 'image')}
                    ${generateDocumentItem('Income Proof', loan.income_proof_url, loan.income_proof_url && loan.income_proof_url.endsWith('.pdf') ? 'pdf' : 'image')}
                    ${generateDocumentItem('Address Proof', loan.address_proof_url, loan.address_proof_url && loan.address_proof_url.endsWith('.pdf') ? 'pdf' : 'image')}
                    ${generateDocumentItem('Utility Bill', loan.utility_bill_url, loan.utility_bill_url && loan.utility_bill_url.endsWith('.pdf') ? 'pdf' : 'image')}
                  </ul>
                  <div id="documentViewer" class="mt-3" style="display: none; height: 500px;">
      ...
      document.querySelectorAll('.view-document-btn').forEach(button => {
        button.addEventListener('click', () => {
          ...
          if (type === 'pdf') {
            ...
            showToast('PDF opened in a new tab. Use the download button to save.', false);
          } else {
    ...
    .catch(error => {
      console.error('Error fetching loan details for documents:', error);
      showToast(`Error fetching documents: ${error.message}. Please try again.`, true);
    });
}

// Helper function to generate document list items
function generateDocumentItem(name, url, type) {
  if (!url || url === 'N/A') {
    return `<li class="list-group-item d-flex justify-content-between align-items-center">
              ${name}: <span class="text-muted">Not Available</span>
            </li>`;
  }
  return `
    <li class="list-group-item d-flex justify-content-between align-items-center">
      ${name}:
      <div>
        <button class="btn btn-pink btn-sm view-document-btn" data-url="${url}" data-type="${type}">
          View
        </button>
        <a href="${url}" class="btn btn-primary btn-sm" download>Download</a>
      </div>
    </li>
  `;
}
